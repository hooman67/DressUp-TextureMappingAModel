...
void hsDeformerShowImage(bool drawMesh){
	...
	for (int i = 0; i < nt; i++)
	{
		...
		line(*hsDeformerImage, cv::Point(verts[tri[0]].vPosition[0], verts[tri[0]].vPosition[1]),
			cv::Point(verts[tri[1]].vPosition[0], verts[tri[1]].vPosition[1]), BLU, 1);

		line(*hsDeformerImage, cv::Point(verts[tri[1]].vPosition[0], verts[tri[1]].vPosition[1]),
			cv::Point(verts[tri[2]].vPosition[0], verts[tri[2]].vPosition[1]), BLU, 1);

		line(*hsDeformerImage, cv::Point(verts[tri[2]].vPosition[0], verts[tri[2]].vPosition[1]),
...
void drawTriagnle(hsTriangle& triangle, bool drawIngreen, Mat*& output){
	...
	if (drawIngreen){
		...
		line(*output, cv::Point(a.x, a.y), cv::Point(b.x, b.y), sGreen, 1);
		line(*output, cv::Point(a.x, a.y), cv::Point(c.x, c.y), sGreen, 1);
		line(*output, cv::Point(b.x, b.y), cv::Point(c.x, c.y), sGreen, 1);
	}
	else{

		line(*output, cv::Point(a.x, a.y), cv::Point(b.x, b.y), sRED, 1);
		line(*output, cv::Point(a.x, a.y), cv::Point(c.x, c.y), sRED, 1);
		line(*output, cv::Point(b.x, b.y), cv::Point(c.x, c.y), sRED, 1);
	}
}
...
int hsfindClosestBodyBoundaryPoint(float x, float y) {
...
void moveSecondDeformControlPoints(){
	...
	while (cur != end){
		...
		float xpos = m_deformer.hsGetVertex(nSelected).vPosition.x();
		float ypos = m_deformer.hsGetVertex(nSelected).vPosition.y();
		int b = hsfindClosestBodyBoundaryPoint(xpos, ypos);
...
int b = hsfindClosestBodyBoundaryPoint(verts[boundaryMeshVertices[i]].vPosition[0], verts[boundaryMeshVertices[i]].vPosition[1]);
...
cout << "boundary vertice size: " << boundaryMeshVertices.size() << " cer: " << cer << "\n";
imshow("kost", kosIm);
waitKey();
*/
...
void findMahdiBoundaryVertices(){
	...
	cout << "found mahdi suggested points\n";
}
...
void hsContourPlotter(){
	...
	for (int i = 0; i < shirtContour[0].size(); i++){
		...
		if (i == 184 || i == 211){
			std::cout << "shirt i: " << i << " (" << shirtContour[0][i].x << ", " << shirtContour[0][i].y << ")" << "\n";
			circle(shirtContIm, shirtContour[0][i], 3, sGreen, -1);
		}
		else{
			std::cout << "shirt i: " << i << " (" << shirtContour[0][i].x << ", " << shirtContour[0][i].y << ")" << "\n";
			circle(shirtContIm, shirtContour[0][i], 3, sRED, -1);
