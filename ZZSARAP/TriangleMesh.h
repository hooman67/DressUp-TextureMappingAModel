 ...
 *  IMesh interface
 */
	virtual VertexID AppendVertex( const Vector3f & vVertex, const Vector3f * pNormal = NULL )
		{ return AppendVertexData( &vVertex, pNormal, false, NULL, &vVertex ); }
	virtual void GetVertex( VertexID vID, Vector3f & vVertex, Vector3f * pNormal = NULL ) const;
	virtual void GetNormal( VertexID vID, Vector3f & vNormal ) const;	
	...
	virtual void GetTriangle( TriangleID tID, Vector3f vTriangle[3], Vector3f * pNormals = NULL  ) const;
	...
	unsigned int
	AppendVertexData( const Vector3f * pVertex, const Vector3f * pNormal, bool bFlipNormal, 
					  const Vector2f * pTextureCoord = NULL, const Vector3f * pColor = NULL );
...
//	void GetNormal( unsigned int nVertex, Wml::Vector3f & n ) const;
	void GetTextureCoords( unsigned int nVertex, Vector2f & vt ) const;
...
//	void GetTriangle( unsigned int nTriangle, Wml::Vector3f * vVertices ) const;
	void GetTextureCoords( unsigned int nTriangle, Vector2f * vCoords ) const;
	void GetTriangleNormals( unsigned int nTriangle, Vector3f * vNormals );
	void GetTriTexCoords( unsigned int nTriangle, Vector2f & uv1, Vector2f & uv2, Vector2f & uv3 ) const;

	void SetTriangle( unsigned int nTriangle, unsigned int triangle[3] );
	void SetVertex( unsigned int nVertex, const Vector3f & vVertex );
	void SetNormal( unsigned int nVertex, const Vector3f & vNormal );
