...
void RigidMeshDeformer2D::SetDeformedHandle( unsigned int nHandle, const Eigen::Vector2f & vHandle )
{
...
{	
	Constraint c(nHandle, Eigen::Vector2f::Zero());
...
void RigidMeshDeformer2D::UnTransformPoint( Eigen::Vector2f & vTransform )
{
	// find triangle
	...
	for ( unsigned int i = 0; i < nTris; ++i ) {
		...
		//hsx
		Wml::Vector2f hsv1, hsv2, hsv3, hsvTransform;
		hsv1[0] = v1[0];
		hsv1[1] = v1[1];
		hsv2[0] = v2[0];
		hsv2[1] = v2[1];
		hsv3[0] = v3[0];
		hsv3[1] = v3[1];
		hsvTransform[0] = vTransform[0];
		hsvTransform[1] = vTransform[1];


		Wml::BarycentricCoords(hsv1, hsv2, hsv3, hsvTransform, fBary1, fBary2, fBary3);
		...
		v3[0] = hsv3[0];
		v3[1] = hsv3[1];
		vTransform[0] = hsvTransform[0];
		vTransform[1] = hsvTransform[1];
...
void RigidMeshDeformer2D::InitializeFromMesh(TriangleMesh * pMesh)
{
	...
	for (unsigned int i = 0; i < nVerts; ++i) {
		...
		Vertex v;
		v.vPosition = Eigen::Vector2f(vVertex.x(), vVertex.y());
...
void RigidMeshDeformer2D::UpdateDeformedMesh(TriangleMesh * pMesh, bool bRigid)
{
	...
	for (unsigned int i = 0; i < nVerts; ++i) {
		...
		pMesh->SetVertex(i, Eigen::Vector3f(vNewPos.x(), vNewPos.y(), 0.0f));
	}
}

//hs
void RigidMeshDeformer2D::buildDeformerMesh() {
	if (hs_m_vInitialVertices.size() < 3)
	{
	...
	for (int i = 0; i < hs_m_vInitialVertices.size(); i++)
	{
		vPoints.push_back(v2d(hs_m_vInitialVertices[i].vPosition[0], hs_m_vInitialVertices[i].vPosition[1]));
	}
	...
	Geometry2D::TriangulateioToOutput(out, vertices, hs_m_vTriangles);
	...
	size_t nTriangle = hs_m_vTriangles.size();
	for (size_t i = 0; i < nTriangle; i++)
	{
		Triangle &t = hs_m_vTriangles[i];
		for (size_t j = 0; j < 3; j++)
		{
			...
			v2f v0 = hs_m_vInitialVertices[t.nVerts[n0]].vPosition;
			v2f v1 = hs_m_vInitialVertices[t.nVerts[n1]].vPosition;
			v2f v2 = hs_m_vInitialVertices[t.nVerts[n2]].vPosition;
...
void RigidMeshDeformer2D::buildTriangleMeshFromDeformer(TriangleMesh * pMesh){
	...
	for (int i = 0; i < hs_m_vInitialVertices.size(); i++){
			...
			rmsmesh::Point vVert;
			vVert.init(hs_m_vInitialVertices[i].vPosition[0], hs_m_vInitialVertices[i].vPosition[1], 0);
			pMesh->AppendVertexData(vVert);
	//		cout << "hs i= " << i << "point = " << hs_m_vInitialVertices[i].vPosition[0] << " and " << hs_m_vInitialVertices[i].vPosition[1] << "\n";
	}

	for (int i = 0; i < hs_m_vTriangles.size(); i++) {

		pMesh->AppendTriangleData(hs_m_vTriangles[i].nVerts);
	}
}
Vertex RigidMeshDeformer2D::hsGetVertex(unsigned int i){
	return m_vInitialVerts[i];
}
unsigned int RigidMeshDeformer2D::hsfindClosestVertex(float x, float y) {
	size_t nVerts = m_vInitialVerts.size();
	float fx, fy;

	double minDistance = std::numeric_limits<double>::max();
	size_t closestVertex = -1;
	for (size_t i = 0; i < nVerts; i++)
	{
		fx = m_vInitialVerts[i].vPosition[0];
		fy = m_vInitialVerts[i].vPosition[1];
	...
	return (closestVertex == -1) ? std::numeric_limits<size_t>::max() : closestVertex;
}
unsigned int RigidMeshDeformer2D::FindHitVertex(float x, float y){
	unsigned int nVerts = m_vDeformedVerts.size();
	float fx, fy;

	for (unsigned int i = 0; i < nVerts; i++)
	{
		fx = m_vDeformedVerts[i].vPosition[0];
		fy = m_vDeformedVerts[i].vPosition[1];
		double disSquare = (x - fx) * (x - fx) + (y - fy) * (y - fy);
		if (disSquare < 25)
		{
			return i;
...
void RigidMeshDeformer2D::UpdateConstraint( Constraint & cons )
{
	...
	if ( found != m_vConstraints.end() ) {
		...
		Constraint temp = *found;
		temp.vConstrainedPos = cons.vConstrainedPos;
		m_vConstraints.erase(*found);
		m_vConstraints.insert(temp);
		//hs all above are mine
...
void RigidMeshDeformer2D::PrecomputeFittingMatrices()
{
	...
	for (unsigned int i = 0; i < nVerts; ++i) {
		Constraint c(i, Eigen::Vector2f::Zero());
...
static Triangle * g_pCurTriangle = NULL;
float g_fErrSum = 0;
...
void RigidMeshDeformer2D::UpdateScaledTriangle( unsigned int nTriangle )
{
	...
	//hsx
	Wml::Vector2f hsvFitted0, hsvFitted1, hsvFitted2;
	hsvFitted0[0] = vFitted0[0];
	hsvFitted0[1] = vFitted0[1];
	hsvFitted1[0] = vFitted1[0];
	hsvFitted1[1] = vFitted1[1];
	hsvFitted2[0] = vFitted2[0];
	hsvFitted2[1] = vFitted2[1];

	Wml::Scale(hsvFitted0, hsvFitted1, hsvFitted2, fScale);

	vFitted0[0] = hsvFitted0[0];
	vFitted0[1] = hsvFitted0[1];
	vFitted1[0] = hsvFitted1[0];
	vFitted1[1] = hsvFitted1[1];
	vFitted2[0] = hsvFitted2[0];
	vFitted2[1] = hsvFitted2[1];
