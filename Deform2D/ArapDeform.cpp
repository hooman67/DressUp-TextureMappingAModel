...
size_t ArapDeform::hsfindClosestVertex(float x, float y) {
	size_t nVerts = m_vInitialVertices.size();
	float fx, fy;
	...
	for (size_t i = 0; i < nVerts; i++)
	{
		fx = m_vInitialVertices[i].vPosition[0];
		fy = m_vInitialVertices[i].vPosition[1];
		double disSquare = (x - fx) * (x - fx) + (y - fy) * (y - fy);
...
void ArapDeform::setVertex(size_t index, GlutPoint position)
{
